...
> 🔗 **[백테스트 결과 조회 (Interactive Viewer)](https://hoioioio.github.io/Crypto_Auto_Trading-BOT/)**
> *설명: 위 링크로 접속하시면 자유롭게 조회 기간(예: 2022.01.01 ~ 2023.12.31)을 설정하여, 해당 기간 동안의 상세 백테스트 결과(수익금, 승률, MDD 등)와 수익 곡선 그래프를 직접 확인하실 수 있습니다.*
...
> 26.5%의 승률임에도 불구하고 **Sortino Ratio 21.01**을 기록한 것은, 추세 발생 시 **피라미딩(Pyramiding)** 통해 수익을 극대화하고 횡보장에서 **KAMA**로 손실을 최소화했음을 보입니다.

### 💎 스트레스 테스트 지표 (Stress Test Metrics)
단순한 전체 기간 누적 데이터를 넘어, 다각적 안정성 검증을 통과했습니다.

| 측정 항목 (Metric) | 수치 / 결과 | 벤치마크 (BTC Buy & Hold) | 비고 |
| :--- | :--- | :--- | :--- |
| **스트레스 테스트 1** <br>*(2022년 루나/FTX 파산)* | **MDD 4.62%**<br>*(수익: -3.48%)* | **76.63%** | 코인 시장 최악의 폭락장에서도 숏(Sell) 포지셔닝 및 KAMA 방어 매커니즘으로 자산 완벽 보호 |
| **스트레스 테스트 2** <br>*(2025.10 마켓 크래시)* | **MDD 0.00%**<br>*(수익: 0.00%)* | **14.62%** | 급락 구간 진입 전 시그널 선반영 및 관망으로 손실 원천 차단 (무결점 방어) |
| **스트레스 테스트 3** <br>*(2026 초반 급락장)* | **MDD 1.06%**<br>*(수익: -1.06%)* | **35.11%** | 역대 최고점 달성 후 쏟아진 폭락(35%)을 불과 1%대의 낙폭으로 커트(Cut-off) |
| **롤링 플로우 MDD** <br>*(Rolling 1-Year, 무작위 1년 투자 시)* | **평균 5.76%** <br>*(최악 11.07%)* | - (비교 생략) | 운에 기댄 과최적화(Overfitting)가 아님을 증명. 언제 시작해도 하방 리스크가 극명히 억제됨 |
| **전고점 회복 기간** <br>*(Underwater Duration)* | **최대 647일** | **1,000일 이상** | 깊은 하락(11%)을 겪더라도 복리와 회전율을 살려 안정적으로 새로운 고점을 갱신해 냄 |
| **상대적 방어력** <br>*(전체 기간 상대 비교)* | **압도적 우위** | **76.63%** | 단순 보유 시 겪어야 하는 파멸적 폭락(76%)을 1/7 이하 수준으로 철저하게 방어 |

### 🚀 백테스트 및 실전 매매 수익 창출 구간 (Upside Potential)
강력한 방어력(MDD)을 바탕으로, 추세 발생 시 피라미딩(Scaling-in)을 통해 극적인 수익을 창출합니다.

| 항목 (Profit Metric) | 수치 (Value) | 대상 종목 | 비고 |
| :--- | :--- | :--- | :--- |
| **단일 거래 최고 수익률** | **+6,411.58%** | **AVAX/USDT** | 21년 8월 불장 당시 피라미딩 진입 / (1회 투입 $20 기준) |
| **단일 거래 최고 수익금** | **$1,150.48** | **ETH/USDT** | 20년 7월 이더리움 변동성 분출 추세 추종 / (1회 투입 $20 기준) |
| **월간 최고 수익 (Best Month)** | **+$1,390.94** | 2020.07 | 한 달 만에 초기 자산(+139%)을 넘어서게 됨 / (1회 투입 $20 기준) |
| **실전 라이브 단일 거래 최고 수익** | **+$134.22** | **BNB/USDT** | 26년 2월 5일 실전 API / (1회 투입 $20 기준) |

### 🔍 라이브 실행 동기화 검증 (Live vs Backtest Audit)
가장 최근 변동 구간(2026.01.29 ~ 현재)에서 **과거 참조 가설(Backtest)과 실제 Binance 선물 API 체결 내역(Live)의 100% 일치 여부**를 교차 검증했습니다.
...
* 💡 **의의:** 캔들 마감 직후 발생하는 초 단위의 API 레이턴시 및 틱 슬리피지(12~36초)를 제외하면, 시스템의 코어 로직이 시뮬레이션에 머물지 않고 실전 거래(Production)에서 오차 없이 **동기화되어 작동함**을 보입니다.
