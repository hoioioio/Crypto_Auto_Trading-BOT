...
본 프로젝트는 단순한 과거 커브피팅(Curve-fitting) 수익률이 아닌, 다양한 제약조건을 걸고 실제 월스트리트 퀀트 수준의 험난한 실전(Reality-check) 구조를 통과함을 입증했습니다. 전체 포트폴리오의 퍼포먼스를 깎아먹는 비트코인(BTC)을 과감히 제거하고 코어 알트코인 파동에 주력했습니다.
...
| **소티노 지수 (Sortino Ratio)** | **12.194** | 하방 위험 대비 탁월한 수익 효율성 (기관급 합격선 초과) |
...
> 27.26%의 승률임에도 불구하고 **Sortino Ratio 12.19**를 기록한 것은, 추세 발생 시 피라미딩(Pyramiding)을 통해 수익을 폭발적으로 불리고 횡보장에서는 KAMA로 손실을 칼같이 억제했음을 의미합니다. 28.45%의 통제된 MDD는 अ떠한 하락장이 오더라도 잔고가 반토막 나지 않게 막아내는 **1.25% Realized Equity Fractional Sizing (실현 자산 기반 동적 베팅)** 구축의 결과입니다.
...
| **단일 거래 최고 수익금** | **$397,898.29** | **ZEC/USDT** | 전체 자산 성장을 폭발시킨 단일 '메가 잭팟' 트렌드-팔로잉 구간 |
| **월간 최고 최고 수익 (Best Month)** | **+$392,224.28** | 2025.09 | 거대한 시장 에너지가 분출하는 달력 (월 단위 초대형 수익금 갱신) |
